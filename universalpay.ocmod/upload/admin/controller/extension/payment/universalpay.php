<?php
class ControllerExtensionPaymentUniversalpay extends Controller{
    ...
    public function index(){
        $this->language->load('extension/payment/universalpay');
...
//             $this->response->redirect($this->url->link('extension/payment/universalpay', 'user_token=' . $this->session->data['user_token'] , true));
        }
        $data = array();
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/payment/universalpay', 'user_token=' . $this->session->data['user_token'], true)
        );
        
        $data['action'] = $this->url->link('extension/payment/universalpay', 'user_token=' . $this->session->data['user_token'], true);
        ...
        if (isset($this->request->post['payment_universalpay_test_token_url'])) {
            $data['payment_universalpay_test_token_url'] = $this->request->post['payment_universalpay_test_token_url'];
        } else {
            $data['payment_universalpay_test_token_url'] = $this->config->get('payment_universalpay_test_token_url');
        }
        if (isset($this->request->post['payment_universalpay_test_payments_url'])) {
            $data['payment_universalpay_test_payments_url'] = $this->request->post['payment_universalpay_test_payments_url'];
        } else {
            $data['payment_universalpay_test_payments_url'] = $this->config->get('payment_universalpay_test_payments_url');
        }
        if (isset($this->request->post['payment_universalpay_test_javascript_url'])) {
            $data['payment_universalpay_test_javascript_url'] = $this->request->post['payment_universalpay_test_javascript_url'];
        } else {
            $data['payment_universalpay_test_javascript_url'] = $this->config->get('payment_universalpay_test_javascript_url');
        }
        if (isset($this->request->post['payment_universalpay_test_cashier_url'])) {
            $data['payment_universalpay_test_cashier_url'] = $this->request->post['payment_universalpay_test_cashier_url'];
        } else {
            $data['payment_universalpay_test_cashier_url'] = $this->config->get('payment_universalpay_test_cashier_url');
        }
        if(empty($this->showUrlFields4Sandbox)){
        ...
        if (isset($this->request->post['payment_universalpay_token_url'])) {
            $data['payment_universalpay_token_url'] = $this->request->post['payment_universalpay_token_url'];
        } else {
            $data['payment_universalpay_token_url'] = $this->config->get('payment_universalpay_token_url');
        }
        if (isset($this->request->post['payment_universalpay_payments_url'])) {
            $data['payment_universalpay_payments_url'] = $this->request->post['payment_universalpay_payments_url'];
        } else {
            $data['payment_universalpay_payments_url'] = $this->config->get('payment_universalpay_payments_url');
        }
        if (isset($this->request->post['payment_universalpay_javascript_url'])) {
            $data['payment_universalpay_javascript_url'] = $this->request->post['payment_universalpay_javascript_url'];
        } else {
            $data['payment_universalpay_javascript_url'] = $this->config->get('payment_universalpay_javascript_url');
        }
        if (isset($this->request->post['payment_universalpay_cashier_url'])) {
            $data['payment_universalpay_cashier_url'] = $this->request->post['payment_universalpay_cashier_url'];
        } else {
            $data['payment_universalpay_cashier_url'] = $this->config->get('payment_universalpay_cashier_url');
        }
        if(empty($this->showUrlFields4Live)){
        ...
        $this->response->setOutput($this->load->view('extension/payment/universalpay', $data));
    }
    
    protected function validate() {
        if (!$this->user->hasPermission('modify', 'extension/payment/universalpay')) {
    ...
    public function install() {
        $this->load->model('extension/payment/universalpay');
        $this->model_extension_payment_universalpay->install();
    }
    
    public function uninstall() {
        $this->load->model('extension/payment/universalpay');
        $this->model_extension_payment_universalpay->uninstall();
    }
    // Legacy 2.0.0
    ...
    public function order() {
        if ($this->config->get('payment_universalpay_status')) {
            ...
            $order_info = $this->model_extension_payment_universalpay->getOrder($this->request->get['order_id']);
            if (!empty($order_info)) {
                ...
                $order_info['total_captured'] = $this->model_extension_payment_universalpay->getTotalCaptured($order_info['universalpay_order_id']);
                $order_info['total_captured_formatted'] = $this->currency->format($order_info['total_captured'], $order_info['currency_code'], 1, true);
                
                $order_info['total_refunded'] = $this->model_extension_payment_universalpay->getTotalRefunded($order_info['universalpay_order_id']);
                ...
                return $this->load->view('extension/payment/universalpay_order', $data);
            }
        }
    }
    public function capture() {
        ...
        if (isset($this->request->post['order_id']) && $this->request->post['order_id'] != '') {
            ...
            $order = $this->model_extension_payment_universalpay->getOrder($this->request->post['order_id']);
            if(empty($order)){
                $json['error'] = true;
                ...
                try {
                    $capture_response = $this->model_extension_payment_universalpay->capture($this->request->post['order_id'], $capture_amount);
                } catch (Exception $e) {
                    $json['error'] = true;
                ...
                if ($capture_response) {
                    $this->model_extension_payment_universalpay->addTransaction($order['universalpay_order_id'], 'payment', $capture_amount);
                    $json['msg'] = 'Capture successfully';
                    $this->model_extension_payment_universalpay->updateCaptureStatus($order['universalpay_order_id'], 1);
                    
                    $this->model_extension_payment_universalpay->updateOrderHistory($this->request->post['order_id'],$this->config->get('payment_universalpay_success_status_id'),'Captured');
                    
                    $json['data'] = array();
                    $json['data']['created'] = $this->model_extension_payment_universalpay->getMysqlNowTime();
    ...
    public function void() {
        ...
        if (isset($this->request->post['order_id']) && $this->request->post['order_id'] != '') {
            ...
            try {
                $void_response = $this->model_extension_payment_universalpay->void($this->request->post['order_id']);
            } catch (Exception $e) {
                $json['error'] = true;
                $json['msg'] = $this->language->get('text_error_connect_gateway');
                $this->response->addHeader('Content-Type: application/json');
            ...
            if ($void_response) {
                $this->model_extension_payment_universalpay->addTransaction($order['universalpay_order_id'], 'void', 0.00);
                $this->model_extension_payment_universalpay->updateVoidStatus($order['universalpay_order_id'], 1);
                
                $this->model_extension_payment_universalpay->updateOrderHistory($this->request->post['order_id'],$this->config->get('payment_universalpay_voided_status_id'),'Voided');
    ...
    public function refund() {
        ...
        if (isset($this->request->post['order_id']) && $this->request->post['order_id'] != '' && isset($this->request->post['amount']) && $this->request->post['amount'] > 0) {
            ...
            $order = $this->model_extension_payment_universalpay->getOrder($this->request->post['order_id']);
            try {
                $refund_response = $this->model_extension_payment_universalpay->refund($this->request->post['order_id'],$refund_amount);
            } catch (Exception $e) {
                $json['error'] = true;
                $json['msg'] = $this->language->get('text_error_connect_gateway');
                $this->response->addHeader('Content-Type: application/json');
            ...
            if ($refund_response) {
                if($refund_response == 2){
                    ...
                    $this->model_extension_payment_universalpay->addTransaction($order['universalpay_order_id'], 'refund', $refund_amount);
                    
                    $total_captured = $this->model_extension_payment_universalpay->getTotalCaptured($order['universalpay_order_id']);
                    $total_refunded = $this->model_extension_payment_universalpay->getTotalRefunded($order['universalpay_order_id']);
                    $refund_status = 0;
                    if ($total_captured == $total_refunded) {
                        $refund_status = 1;
                        $this->model_extension_payment_universalpay->updateRefundStatus($order['universalpay_order_id'], $refund_status);
                        $this->model_extension_payment_universalpay->updateOrderHistory($this->request->post['order_id'],$this->config->get('payment_universalpay_refunded_status_id'),'Refunded');
                    }
