<?php
require_once modification(DIR_SYSTEM . 'library/universalpay/payments.php');
class ControllerExtensionPaymentUniversalpay extends Controller {
	public function index() {
		if ($order_info = $this->model_checkout_order->getOrder($this->session->data['order_id'])) {
			...
			$post_data = array();
			$post_data['merchantId'] = trim($this->config->get('payment_universalpay_clientid'));
			$post_data['merchantTxId'] = substr(md5(uniqid(mt_rand(), true)), 0, 20);
			$post_data['password'] = trim($this->config->get('payment_universalpay_password'));
			$post_data['brandId'] = trim($this->config->get('payment_universalpay_brandid'));
			
			$post_data['customerId'] = $order_info['customer_id'];
			$post_data['allowOriginUrl'] = $this->getAllowOriginUrl();
			$post_data['merchantLandingPageUrl'] = html_entity_decode($this->url->link('extension/payment/universalpay/redirectBack', 'order_id='.$this->session->data['order_id'],true), ENT_QUOTES, 'UTF-8');
			$post_data['merchantNotificationUrl'] = html_entity_decode($this->url->link('extension/payment/universalpay/callback', 'order_id='.$this->session->data['order_id'],true), ENT_QUOTES, 'UTF-8');
...
// 			$post_data['country'] = $order_info['payment_iso_code_2'];
			...
			try {
			    ...
			    $payment_universalpay_pay_action = $this->config->get('payment_universalpay_pay_action'); //1 refer to auth, 0 refer to purchase
			    if ($payment_universalpay_pay_action){
			    ...
			    return $this->load->view('extension/payment/universalpay', $data);
			}
			...
			if(isset($res->result) && $res->result == 'success'){
			    ...
			    $data['merchantId'] = trim($this->config->get('payment_universalpay_clientid'));
			    
			    $data['button_confirm'] = $this->language->get('button_confirm');
			    
			    $testmode = $this->config->get('payment_universalpay_testmode');
			    if ($testmode) $data['action'] = $this->config->get('payment_universalpay_test_cashier_url');
			    else $data['action'] = $this->config->get('payment_universalpay_cashier_url');
			    
			    $data['java_script_url'] = $this->config->get('payment_universalpay_test_javascript_url');
			    $data['cashier_url'] = $this->config->get('payment_universalpay_test_cashier_url');
			}else{
			    $data['error'] = $this->language->get('text_error_message');
			}
			if($this->config->get('payment_universalpay_pay_type') == 1){
			    //iframe payment mode
			    $data['post_url'] = $post_data['merchantLandingPageUrl'];
			    return $this->load->view('extension/payment/universalpay_iframe', $data);
			}else if($this->config->get('payment_universalpay_pay_type') == 2){
	...
	private function initConfig(){
	    Payments\Config::$MerchantId = trim($this->config->get('payment_universalpay_clientid'));
	    Payments\Config::$Password = trim($this->config->get('payment_universalpay_password'));
	    $testmode = $this->config->get('payment_universalpay_testmode');
	    if ($testmode){
	        Payments\Config::$TestUrls["SessionTokenRequestUrl"] = $this->config->get('payment_universalpay_test_token_url');
	        Payments\Config::$TestUrls["PaymentOperationActionUrl"] = $this->config->get('payment_universalpay_test_payments_url');
	        Payments\Config::$TestUrls["JavaScriptUrl"] = $this->config->get('payment_universalpay_test_javascript_url');
	        Payments\Config::$TestUrls["BaseUrl"] = $this->config->get('payment_universalpay_test_cashier_url');
	        Payments\Config::test();
	    }else{
	        Payments\Config::$ProductionUrls["SessionTokenRequestUrl"] = $this->config->get('payment_universalpay_token_url');
	        Payments\Config::$ProductionUrls["PaymentOperationActionUrl"] = $this->config->get('payment_universalpay_payments_url');
	        Payments\Config::$ProductionUrls["JavaScriptUrl"] = $this->config->get('payment_universalpay_javascript_url');
	        Payments\Config::$ProductionUrls["BaseUrl"] = $this->config->get('payment_universalpay_cashier_url');
	...
	private function checkStatus($order_id,$merchantTxId){
	    ...
	    try {
	        ...
	        if(!isset($result->result) || $result->result != 'success'){
	            ...
	            if($this->model_extension_payment_universalpay->getTotalOrderHistories($order_id)){
	                //the order status has been checked already
	                $this->response->redirect($this->url->link('checkout/failure'));
	            }else{
	                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_universalpay_failed_status_id'),'Failed');
	                $this->response->redirect($this->url->link('checkout/failure'));
	            }
	            exit;
	        }else{
	            if($this->model_extension_payment_universalpay->hasOrder($order_id)){
	                //the order status has been checked already
	            ...
	            if($result->status == 'SET_FOR_CAPTURE' || $result->status == 'CAPTURED'){
	                ...
	                $universalpay_order_id = $this->model_extension_payment_universalpay->addOrder($order_data);
	                $this->model_extension_payment_universalpay->addTransaction($universalpay_order_id,'payment', $order_info);
	                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_universalpay_success_status_id'),'Paid');
	                $this->response->redirect($this->url->link('checkout/success'));
	            }else if($result->status == 'NOT_SET_FOR_CAPTURE'){
	                // AUTH was successful
	                $universalpay_order_id = $this->model_extension_payment_universalpay->addOrder($order_data);
	                $this->model_extension_payment_universalpay->addTransaction($universalpay_order_id,'auth', $order_info);
	                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_universalpay_auth_status_id'),'Authorized');
	                $this->response->redirect($this->url->link('checkout/success'));
	            }else{
